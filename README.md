...
#  СПИСОК ЗАДАЧ
 ...
 + [Две функции](#две-функции)
 + [Значения определенного поля](#значения-определенного-поля)
 + [Фильтер](#фильтер)
 + [Число свойств](#число-свойств)
 + [Сортировка по убыванию](#сортировка-по-убыванию)
 + [Поверхностное копирование](#поверхностное-копирование)
...
Начальное число, с которого начинать отсчет, и шаг, задается при создании генератора. Шаг можно 
не указывать, тогда он будет равен одному. Начальное значение по умолчанию равно 0. Генераторов можно 
создать сколько угодно. Также, нужна функция take(gen, x) которая вызвает функцию gen заданное
...
где gen — функция-генератор вроде той, что была в задачи гениратор.
...
```
Напиши программу, которая отберет и выведет N самых населенных городов
по убыванию числа жителей.
```
...
mostPopulatedCities.sort(sortingCities)
.map(({city})=> city);
```
</p>
</details>
...
```
Напиши функцию неглубокого копирования объектов.Функция неглубокого копирования
должна создавать новый объект, и копировать в него элементы из старого.
```
